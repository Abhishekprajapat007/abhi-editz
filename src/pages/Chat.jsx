...
function Chat() {
  ...
  useEffect(() => {
    ...
    const loadChat = async () => {
      ...
      const { data, error } = await supabase
        .from('messages')
        .select('id, sender_id, receiver_id, message, created_at, updated_at, is_read')
  ...
  user.id === project.client_id
    ? (await supabase.rpc('get_admin_id')).data
    : project.client_id
    ...
    const { data, error } = await supabase
      ...
      .select('id, sender_id, receiver_id, message, created_at, updated_at')
      .single()
  ...
  if (!projectId || !project) {
    return (
      <div className="dashboard-page">
        <div className="dashboard-card">
          <h2>Project Chat</h2>
          <p>Please open chat from a project.</p>
